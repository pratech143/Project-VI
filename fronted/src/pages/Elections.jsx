...
import { Vote, MapPin, Calendar, Users, RefreshCcw, ChevronDown, ChevronUp, Award } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
        ...
        <p className="text-gray-400 max-w-2xl mx-auto">View and participate in current elections. Your vote matters.</p>
      </motion.div>
      <AnimatePresence>
        {elections.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {elections.map((election) => (
              <motion.div key={election.election_id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                <Card className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 hover:border-indigo-500/50 transition-all duration-300 w-full">
                  <CardHeader>
                    <CardTitle className="text-2xl font-bold text-white flex justify-between items-center">
                      {election.name}
                      <Button variant="ghost" size="icon" onClick={() => dispatch(fetchElections({ voter_id }))}>
                  ...
                  <CardContent>
                    ...
                    <div className="flex items-center text-gray-300 mt-4">
                      ...
                      <span>{format(new Date(election.start_date), 'PPP')} - {format(new Date(election.end_date), 'PPP')}</span>
                    </div>
                    <div className="mt-6">
                      <h3 className="text-lg font-semibold text-white flex items-center">
                        <Award className="w-5 h-5 mr-2 text-indigo-400" /> Candidates
                      </h3>
                      <div className="mt-4 space-y-2">
                        {Object.keys(election.candidates).map((post) => (
                          <div key={post} className="bg-gray-700/30 rounded-lg p-4">
                            <button onClick={() => toggleExpand(post)} className="flex justify-between w-full text-left text-indigo-400 font-medium">
                              {post}
                              {expandedPost === post ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </button>
                            <AnimatePresence>
                              {expandedPost === post && (
                                  ...
                                  {election.candidates[post].length > 0 ? (
                                    <div className="space-y-2">
                                      {election.candidates[post].map((candidate) => (
                                        <div key={candidate.candidate_id} className="flex items-center justify-between bg-gray-800/50 rounded-lg p-3 hover:bg-gray-700/50 transition-colors">
                                          <div>
                                            <p className="text-white font-medium">{candidate.candidate_name}</p>
                                            <p className="text-gray-400 text-sm">{candidate.party_name}</p>
                                          </div>
                                        </div>
                                      ))}
                                    </div>
                                  ) : (
                                    <p className="text-gray-400 italic mt-2">No candidates registered</p>
                                  )}
                                </motion.div>
                              )}
                            </AnimatePresence>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-16">
            ...
            <Button onClick={() => dispatch(fetchElections({ voter_id }))} className="bg-indigo-600 hover:bg-indigo-700">
