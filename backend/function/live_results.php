...
while ($row = $result->fetch_assoc()) {
    ...
    $encrypted_candidate_id = $row['encrypted_candidate_id'];

    $decrypted_candidate_id = decryptData($encrypted_candidate_id);

    error_log("Decrypted Candidate ID: " . $decrypted_candidate_id);
    error_log("Original Candidate ID: " . $candidate_id);

    if (!$decrypted_candidate_id) {
        error_log("Decryption failed for encrypted_candidate_id: " . $encrypted_candidate_id);
        continue;
    }

    if ((int)$decrypted_candidate_id !== (int)$candidate_id) {
        error_log("Mismatch! Decrypted Candidate ID ($decrypted_candidate_id) does not match stored ID ($candidate_id).");
        continue;
    }
